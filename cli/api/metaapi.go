...
func (i *Inode) Equals(than *Inode) bool {
	if i == nil && than == nil {
		return true
	}
	if (i != nil && than == nil) || (i == nil && than != nil ) {
		return false
	}
	if i.Inode != than.Inode || i.Type != than.Type || i.Uid != than.Uid || i.Gid != than.Gid || i.Size != than.Size || i.Generation != than.Generation ||
		i.CreateTime != than.CreateTime || i.AccessTime != than.AccessTime || i.ModifyTime != than.ModifyTime ||
		i.NLink != than.NLink || i.Flag != than.Flag || i.Reserved != than.Reserved {
		return false
	}
	if len(i.Extents) != len(than.Extents) {
		return false
	}
	if len(i.Extents) == 0 {
		return true
	}
	equal := bytes.Compare(i.LinkTarget, than.LinkTarget)
	log.LogErrorf("equal %v", equal)
	return reflect.DeepEqual(i.Extents, than.Extents)
}
