...
int cfs_parse_time(const char *str, size_t len, struct timespec64 *time)
{
	...
	const char *pos, *plus, *zulu, *dot, *secend;
	int ret;
	...
	if (ret < 0) return ret;
	str = pos + 1;
	plus = strnchr(str, end - str, '+');
	zulu = strnchr(str, end - str, 'Z');
	dot = strnchr(str, end - str, '.');
	secend = end;
	if (dot && dot < secend) secend = dot;
	if (plus && plus < secend) secend = plus;
	if (zulu && zulu < secend) secend = zulu;
	ret = cfs_kstrntou32(str, secend - str, 10, &sec);
	if (ret < 0) return ret;
	if (plus) {
		str = plus + 1;
		pos = strnchr(str, end - str, ':');
		if (pos) {
			...
			if (ret < 0) return ret;
			str = pos + 1;
			ret = cfs_kstrntou32(str, end - str, 10, &tz_min);
			if (ret < 0) return ret;
		}
