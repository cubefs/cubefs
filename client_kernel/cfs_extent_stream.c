				  ...
				  struct cfs_packet *packet)
{
	...
	if (err < 0) {
		cfs_log_error(es->ec->log, "rdma(%s) reqid(%llu) create error %d\n",
	...
	if (err < 0) {
		cfs_log_error(es->ec->log, "rdma(%s) reqid(%llu) send packet error %d\n",
			      cfs_pr_addr_rdma(host, es->rdma_port), be64_to_cpu(packet->request.hdr.req_id), err);
		goto out;
	}
	...
	if (err) {
		cfs_log_error(es->ec->log, "rdma(%s) reqid(%llu) recv packet error %d\n",
			      cfs_pr_addr_rdma(host, es->rdma_port), be64_to_cpu(packet->request.hdr.req_id), err);
		goto out;
	}
			     ...
			     struct cfs_packet *packet)
{
	...
	if (err) {
		cfs_log_error(es->ec->log, "socket(%s) reqid(%llu) create error %d\n",
	...
	if (err) {
		cfs_log_error(es->ec->log,
			      "socket(%s) reqid(%llu) set recv timeout error %d\n",
			      cfs_pr_addr(host), be64_to_cpu(packet->request.hdr.req_id), err);
		goto out;
	}
	...
	if (err < 0) {
		cfs_log_error(es->ec->log, "socket(%s) reqid(%llu) send packet error %d\n",
			      cfs_pr_addr(host), be64_to_cpu(packet->request.hdr.req_id), err);
		goto out;
	}
	...
	if (err) {
		cfs_log_error(es->ec->log, "socket(%s) reqid(%llu) recv packet error %d\n",
			      cfs_pr_addr(host), be64_to_cpu(packet->request.hdr.req_id), err);
		goto out;
	}
...
retry:
	...
	if (es->enable_rdma && (packet->request.hdr.opcode == CFS_OP_STREAM_WRITE || packet->request.hdr.opcode == CFS_OP_STREAM_RANDOM_WRITE)) {
...
retry:
	...
	if (ret < 0) {
		cfs_log_error(es->ec->log, "ino(%llu) reqid(%llu) create extent error %d\n",
	...
	if (ret < 0) {
		cfs_log_error(es->ec->log,
			      "ino(%llu) reqid(%llu) create extent reply error code 0x%x\n",
				     ...
				     struct cfs_page_iter *iter)
{
	...
	while (send_bytes < total_bytes) {
		...
		if (IS_ERR(writer)) {
			cfs_log_error(es->ec->log, "extent_stream_get_writer failed\n");
			return -EIO;
		}
...
static void extent_read_pages_reply_cb(struct cfs_packet *packet)
{
	...
	if (packet->error) {
		err = packet->error;
		cfs_log_error(log, "req_id: %llu, packet error: %d\n", be64_to_cpu(packet->request.hdr.req_id), err);
	} else {
		err = -cfs_parse_status(packet->reply.hdr.result_code);
	}

	if (err)
		cfs_log_error(log, "ino(%llu) req_id(%llu) reply error %d\n", es->ino, be64_to_cpu(packet->request.hdr.req_id), err);
...
static int extent_write_iter_random(struct cfs_extent_stream *es, struct cfs_extent_io_info *io_info, struct iov_iter *iter)
{
	struct cfs_data_partition *dp;
	struct cfs_packet *packet;
	size_t w_len;
	...
	while (send_bytes < io_info->size) {
		...
		if (!packet) {
			ret = -ENOMEM;
			cfs_log_error(es->ec->log, "cfs_extent_packet_new return NULL\n");
			goto out;
		}
...
static int extent_write_iter_tiny(struct cfs_extent_stream *es, struct cfs_extent_io_info *io_info, struct iov_iter *iter)
{
	struct cfs_data_partition *dp;
	struct cfs_packet *packet;
	struct cfs_packet_extent extent;
...
retry:
	if (retry_cnt == 0) {
		cfs_log_error(es->ec->log, "extent_write_iter_tiny reqid(%llu) retry_cnt(%u) expired\n", be64_to_cpu(packet->request.hdr.req_id), retry_cnt);
		cfs_packet_release(packet);
		return ret;
	}
...
static int extent_write_iter_normal(struct cfs_extent_stream *es,
				     struct cfs_extent_io_info *io_info,
...
static int cfs_extent_write_iter(struct cfs_extent_stream *es, struct cfs_extent_io_info *io_info, struct iov_iter *iter) {
	int ret = 0;
...
static int cfs_extent_read_iter(struct cfs_extent_stream *es,
...
{
	LIST_HEAD(io_info_list);
	struct cfs_extent_io_info *io_info = NULL;
	int ret = 0;
	ssize_t io_bytes = 0;
	...
	while (!list_empty(&io_info_list)) {
		...
		list_del(&io_info->list);
		if (type == WRITE) {
			ret = cfs_extent_write_iter(es, io_info, iter);
		} else {
			ret = cfs_extent_read_iter(es, io_info, iter);
		}
		cfs_extent_io_info_release(io_info);
		if (ret < 0) {
			mutex_unlock(&es->lock_io);
