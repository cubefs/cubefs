...
build_blobstore_bench() {
    pushd $SrcPath > /dev/null
    go build ${MODFLAGS} -gcflags=all=-trimpath="${SrcPath}" -asmflags=all=-trimpath="${SrcPath}" -ldflags="${LDFlags}" -o "${BuildBinPath}/blobstore/blobstore-bench" "${SrcPath}/blobstore/cli/bench"
    popd > /dev/null
}
...
build_blobstore() {
    ...
    build_clustermgr && build_blobnode && build_access && build_scheduler && build_proxy && build_blobstore_cli && build_blobstore_bench && echo "success" || echo "failed"
    popd >/dev/null
}
