...
class SlotCache:
    ...
    def insert_cube_item(self, file_path, encode_bytes):
        ...
        with slot_cache_lock:
            ...
            if not self.is_empty_avali_time(self.shard_bytes_arr[write_offset:write_offset+8]):
                ...
                print("slot_index:{} write_offset:{} has exsit "
                      "item:[avali_time:{},path:{}]".format(self.slot_index,write_offset,exsit_avali_time,exsit_file_path))
                return None
        ...
        print("pid:{} slot_index:{} write_offset:{} file_path:{} item_meta:{} has cached".format(os.getpid(),self.slot_index,write_offset,file_path,item_meta))
        return item_meta

    def delete_cube_item_shard_memory(self, file_path, item_meta):
    ...
    def get_cube_item(self, expect_file_path, item_meta):
        ...
        if self.is_empty_avali_time(data[offset:offset + 8]):
            print("expect_file_path:{} avali_time:{} "
                  "".format(expect_file_path,int.from_bytes(data[offset:offset + 8],byteorder='big')))
            return None
...
class ShardMemory:
    ...
    def delete_cube_item_shard_memory(self, file_path, item_meta):
        slot_index, offset, size = item_meta
        return self.slot_caches[slot_index].delete_cube_item_shard_memory(file_path, item_meta)
