...
cd /go/src/github.com/cubefs/cubefs/script;
bash ./build.sh &>> /tmp/cfs_build_output
if [[ $? -eq 0 ]]; then
    echo -e "\033[32mdone\033[0m";
    mv /bin/cfs-server /go/src/github.com/cubefs/cubefs/docker/bin/cfs-server;
else
...
cd /go/src/github.com/cubefs/cubefs/bin;
bash ./build.sh &>> /tmp/cfs_build_output
if [[ $? -eq 0 ]]; then
    echo -e "\033[32mdone\033[0m";
...
echo -n 'Building CubeFS Bcache ... ';
cd /go/src/github.com/cubefs/cubefs/blockcache;
bash ./build.sh &>> /tmp/cfs_build_output;
if [[ $? -eq 0 ]]; then
    echo -e "\033[32mdone\033[0m";
    mv cfs-bcache /go/src/github.com/cubefs/cubefs/docker/bin/;
else
    echo -e "\033[31mfail\033[0m";
    failed=1
fi
