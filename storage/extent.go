...
func (e *Extent) Write(data []byte, offset, size int64, crc uint32, crcFunc UpdateCrcFunc, isUpdateSize bool) (err error) {
	if IsTinyExtent(e.extentID) {
	...
	if writeSize, err = e.file.WriteAt(data[:size], int64(offset)); err != nil {
		return
	}
