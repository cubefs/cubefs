...
// key=#renewMpStoreModeTask,value=json.Marshal(ClusterPlan)
func (c *Cluster) syncAddRenewMpStoreModeTask(task *proto.RenewMpStoreModePlan) (err error) {
	return c.putRenewMpStoreModeTaskInfo(opSyncAddBalanceTask, task)
}

func (c *Cluster) syncUpdateRenewMpStoreModeTask(task *proto.RenewMpStoreModePlan) (err error) {
	return c.putRenewMpStoreModeTaskInfo(opSyncUpdateBalanceTask, task)
}

func (c *Cluster) putRenewMpStoreModeTaskInfo(opType uint32, task *proto.RenewMpStoreModePlan) error {
	balanceTask, err := c.buildRenewMpStoreModeTaskRaftCmd(opType, task)
	if err != nil {
		return err
	}
	return c.submit(balanceTask)
...
func (c *Cluster) buildRenewMpStoreModeTaskRaftCmd(opType uint32, task *proto.RenewMpStoreModePlan) (*RaftCmd, error) {
	balanceTask := new(RaftCmd)
	balanceTask.Op = opType
	balanceTask.K = renewMpStoreModeTaskKey
	var err error
	if balanceTask.V, err = json.Marshal(task); err != nil {
		return nil, fmt.Errorf("balance task op(%d) encode err: %s", opType, err.Error())
	}
...
func (c *Cluster) loadRenewMpStoreModeTask() (*proto.RenewMpStoreModePlan, error) {
	result, err := c.fsm.store.GetByKey([]byte(renewMpStoreModeTaskKey))
	if err != nil {
		return nil, fmt.Errorf("loadRenewMpStoreModeTask GetByKey err: %s", err.Error())
	}

	if len(result) == 0 {
		return nil, proto.ErrNoRenewMpStoreModeTask
	}

	task := new(proto.RenewMpStoreModePlan)
	if task.Msg == nil {
		task.Msg = make(map[uint64]string)
	}
	err = json.Unmarshal(result, task)
	if err != nil {
		return nil, fmt.Errorf("loadRenewMpStoreModeTask decode json err: %s", err.Error())
	}

	return task, nil
}

func (c *Cluster) syncDeleteRenewMpStoreModeTask() error {
	err := c.fsm.store.DelByKey([]byte(renewMpStoreModeTaskKey), true)
	if err != nil {
		log.LogErrorf("DelByKey err: %s", err.Error())
	}
