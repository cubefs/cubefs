...
func (t *topology) allocZonesForDataNode(zoneNum, replicaNum int, excludeZone []string, mediaType uint32) (zones []*Zone, err error) {
	...
	if mediaType != proto.MediaType_Unspecified {
		...
		for _, zone := range zones {
			log.LogInfof("######### allocZonesForDataNode check picking, zone:%v mediaType(%v)", zone.name, mediaType) //TODO:tangjingyu del
			if zone.dataMediaType == mediaType {
				log.LogInfof("######### allocZonesForDataNode pick zone:%v mediaType(%v)", zone.name, mediaType) //TODO:tangjingyu del
