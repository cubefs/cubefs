...
func (c *Cluster) checkZoneArgs(crossZone bool, zoneName string) error {
	if crossZone && c.t.zoneLen() <= 1 {
		return fmt.Errorf("cluster only has one zone,can't cross zone")
	}
...
func (m *Server) checkCreateReq(req *createVolReq) (err error) {
	...
	if err = m.cluster.checkZoneArgs(req.crossZone, req.zoneName); err != nil {
		return err
	}
	...
	case proto.VolumeTypeCold:
		...
		if args.ebsCapacity == 0 {
			return fmt.Errorf("ebs capacity must greater than 0, now %d", args.ebsCapacity)
		}
