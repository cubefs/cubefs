...
func mockMetaNodes() (nodes *sync.Map) {
	...
	metaNode.ID = 1
	metaNode.MaxMemAvailWeight = 64*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 64*util.GB
	metaNode.Ratio = 0.5
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		...
		{Total:1*util.TB, Used:0, Path:"/data1", UsageRatio:0},
		{Total:1*util.TB, Used:120*util.GB, Path:"/data2", UsageRatio:float64(120) / float64(1024)},
	}
	nodes.Store(metaNode.Addr, metaNode)

	metaNode = newMockMetaNode("192.168.0.2", "test", "test", "3.0.0")
	metaNode.ID = 2
	metaNode.MaxMemAvailWeight = 128*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 0
	metaNode.Ratio = 0
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:500*util.GB, Used:12*util.GB, Path:"/data0", UsageRatio:float64(12) / float64(500)},
		{Total:500*util.GB, Used:120*util.GB, Path:"/data1", UsageRatio:float64(120) / float64(500)},
	}
	nodes.Store(metaNode.Addr, metaNode)

	metaNode = newMockMetaNode("192.168.0.3", "test", "test", "3.0.0")
	metaNode.ID = 3
	metaNode.MaxMemAvailWeight = 126*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 2*util.GB
	metaNode.Ratio = 0.016
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:500*util.GB, Used:300*util.GB, Path:"/data0", UsageRatio:0.6},
		{Total:500*util.GB, Used:0, Path:"/data1", UsageRatio:0},
	}
	nodes.Store(metaNode.Addr, metaNode)

	metaNode = newMockMetaNode("192.168.0.4", "test", "test", "3.0.0")
	metaNode.ID = 4
	metaNode.MaxMemAvailWeight = 16*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 108*util.GB
	metaNode.Ratio = 0.843
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:4*util.TB, Used:1*util.TB, Path:"/data0", UsageRatio:0.25},
	}
	nodes.Store(metaNode.Addr, metaNode)

	metaNode = newMockMetaNode("192.168.0.5", "test", "test", "3.0.0")
	metaNode.ID = 5
	metaNode.MaxMemAvailWeight = 118*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 10*util.GB
	metaNode.Ratio = 0.078
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:4*util.TB, Used:3*util.TB, Path:"/data0", UsageRatio:0.75},
	}
	nodes.Store(metaNode.Addr, metaNode)

	metaNode = newMockMetaNode("192.168.0.6", "test", "test", "3.0.0")
	metaNode.ID = 6
	metaNode.MaxMemAvailWeight = 40*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 88*util.GB
	metaNode.Ratio = 0.688
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:4*util.TB, Used:120*util.GB, Path:"/data0", UsageRatio:float64(120) / float64(1024*4)},
	}
	nodes.Store(metaNode.Addr, metaNode)
...
func TestMetaNode_SelectNodeCase02(t *testing.T) {
	...
	metaNode := newMockMetaNode("192.168.0.1", "test", "test", "3.0.0")
	metaNode.ID = 1
	metaNode.MaxMemAvailWeight = 20*util.GB
	metaNode.Total = 128*util.GB
	metaNode.Used = 108*util.GB
	metaNode.Ratio = 0.844
	metaNode.RocksdbDisks = []*proto.MetaNodeDiskInfo{
		{Total:4*util.TB, Used:2*util.TB, Path:"/data0", UsageRatio: 0.5},
	}
