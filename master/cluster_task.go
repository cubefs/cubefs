...
func (c *Cluster) migrateMetaPartition(srcAddr, targetAddr string, mp *MetaPartition, dstStoreMode proto.StoreMode) (err error) {
	...
	if targetAddr != "" {
	...
	} else if _, newPeers, err = ns.getAvailMetaNodeHosts(param, dstStoreMode, 0); err != nil {
		...
		if _, newPeers, err = zone.getAvailNodeHosts(nodeType, param, 0); err != nil {
			...
			if _, newPeers, err = c.getHostFromNormalZone(nodeType, excludeZone, 1, "", proto.MediaType_Unspecified, param, 0); err != nil {
				goto errHandler
			}
		}
	}
