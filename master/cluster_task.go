...
func (c *Cluster) migrateMetaPartition(srcAddr, targetAddr string, mp *MetaPartition, dstStoreMode proto.StoreMode) (err error) {
	...
	if targetAddr != "" {
	...
	} else if _, newPeers, err = ns.getAvailMetaNodeHosts(param, dstStoreMode, false); err != nil {
		...
		if _, newPeers, err = zone.getAvailNodeHosts(nodeType, param, false); err != nil {
			...
			if _, newPeers, err = c.getHostFromNormalZone(nodeType, excludeZone, 1, "", proto.MediaType_Unspecified, param, false); err != nil {
				goto errHandler
			}
		}
	}
