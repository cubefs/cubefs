...
begin:
	...
	log.LogDebugf("Streamer write enter: ino(%v) offset(%v) size(%v) storageClass(%v)",
		s.inode, offset, size, storageClass)
...
func (s *Streamer) doAppendWrite(data []byte, offset, size int, direct bool, reUseEk bool, storageClass uint32) (total int, err error, status int32) {
	var (
		ek        *proto.ExtentKey
		storeMode int
	)
