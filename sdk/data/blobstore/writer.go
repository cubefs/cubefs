...
func (writer *Writer) WriteWithoutPool(ctx context.Context, offset int, data []byte) (size int, err error) {
	...
	span.Debugf("TRACE blobStore WriteWithoutPool Enter: ino(%v) offset(%v) len(%v) fileSize(%v)",
		writer.ino, offset, len(data), writer.CacheFileSize())

	if len(data) > MaxBufferSize || offset != writer.CacheFileSize() {
		span.Errorf("TRACE blobStore WriteWithoutPool error,may be len(%v)>512MB,offset(%v)!=fileSize(%v)",
			len(data), offset, writer.CacheFileSize())
		err = syscall.EOPNOTSUPP
		return
	}
	// write buffer
	span.Debugf("TRACE blobStore WriteWithoutPool: ino(%v) offset(%v) len(%v)",
...
func (writer *Writer) Write(ctx context.Context, offset int, data []byte, flags int) (size int, err error) {
	...
	span.Debugf("TRACE blobStore Write Enter: ino(%v) offset(%v) len(%v) flags&proto.FlagsAppend(%v) fileSize(%v)", writer.ino, offset, len(data), flags&proto.FlagsAppend, writer.CacheFileSize())

	if len(data) > MaxBufferSize || flags&proto.FlagsAppend == 0 || offset != writer.CacheFileSize() {
		span.Errorf("TRACE blobStore Write error,may be len(%v)>512MB,flags(%v)!=flagAppend,offset(%v)!=fileSize(%v)", len(data), flags&proto.FlagsAppend, offset, writer.CacheFileSize())
		err = syscall.EOPNOTSUPP
		return
	}
	// write buffer
	span.Debugf("TRACE blobStore Write: ino(%v) offset(%v) len(%v) flags&proto.FlagsSyncWrite(%v)", writer.ino, offset, len(data), flags&proto.FlagsSyncWrite)
...
func (writer *Writer) doParallelWrite(ctx context.Context, data []byte, offset int) (size int, err error) {
	span := proto.SpanFromContext(ctx)
	span.Debugf("TRACE blobStore doDirectWrite: ino(%v) offset(%v) len(%v)", writer.ino, offset, len(data))
	writer.Lock()
	defer writer.Unlock()
	wSlices := writer.prepareWriteSlice(offset, data)
	span.Debugf("TRACE blobStore prepareWriteSlice: wSlices(%v)", wSlices)
	...
	for i := 0; i < sliceSize; i++ {
		if wErr := <-writer.err; wErr != nil {
			span.Errorf("slice write error,ino(%v) fileoffset(%v) sliceSize(%v) err(%v)",
	...
	for _, wSlice := range wSlices {
		writer.cacheLevel2(ctx, wSlice)
	}

	return
}

func (writer *Writer) cacheLevel2(ctx context.Context, wSlice *rwSlice) {
	if writer.cacheAction == proto.RWCache && (wSlice.fileOffset+uint64(wSlice.size)) < uint64(writer.cacheThreshold) || writer.fileCache {
		...
		go writer.asyncCache(ctx, writer.ino, offSet, buf)
	}
}
...
func (writer *Writer) WriteFromReader(ctx context.Context, reader io.Reader, h hash.Hash) (size uint64, err error) {
	...
	writeBuff := func() {
		...
		span.Debugf("writeBuff: bufSize(%v), leftToWrite(%v), err(%v)", bufSize, leftToWrite, err)
		if bufSize == writer.blockSize || (leftToWrite == 0 && err == io.EOF) {
			...
			if (err == nil || err == io.EOF) && h != nil {
				h.Write(wSlice.Data)
				span.Debugf("writeBuff: bufSize(%v), md5", bufSize)
			}
			writer.wg.Add(1)
...
LOOP:
	for {
		...
		for leftToWrite > 0 {
			span.Debugf("WriteFromReader: leftToWrite(%v), err(%v)", leftToWrite, err)
			writer.RLock()
		...
		if err == io.EOF {
			span.Debugf("WriteFromReader: EOF")
			if len(writer.buf) > 0 {
				writeBuff()
			}
			err = nil
			...
			if err != nil {
				span.Errorf("slice write error,ino(%v) fileoffset(%v)  sliceSize(%v) err(%v)", writer.ino, wErr.fileOffset, wErr.size, err)
			}
			break
		}
	}

	span.Debugf("WriteFromReader before sort: %v", oeks)
...
func (writer *Writer) doBufferWriteWithoutPool(ctx context.Context, data []byte, offset int) (size int, err error) {
	span := proto.SpanFromContext(ctx)
	span.Debugf("TRACE blobStore doBufferWriteWithoutPool Enter: ino(%v) offset(%v) len(%v)", writer.ino, offset, len(data))
	...
	for dataSize > 0 {
		...
		span.Debugf("TRACE blobStore doBufferWriteWithoutPool: ino(%v) writer.fileSize(%v) writer.fileOffset(%v) position(%v) freeSize(%v)", writer.ino, writer.fileSize, writer.fileOffset, position, freeSize)
		writer.buf = append(writer.buf, data[position:position+freeSize]...)
		span.Debugf("TRACE blobStore doBufferWriteWithoutPool:ino(%v) writer.buf.len(%v)", writer.ino, len(writer.buf))
		position += freeSize
		dataSize -= freeSize
	...
	span.Debugf("TRACE blobStore doBufferWriteWithoutPool Exit: ino(%v) writer.fileSize(%v) writer.fileOffset(%v)", writer.ino, writer.fileSize, writer.fileOffset)
	return size, nil
}

func (writer *Writer) doBufferWrite(ctx context.Context, data []byte, offset int) (size int, err error) {
	span := proto.SpanFromContext(ctx)
	span.Debugf("TRACE blobStore doBufferWrite Enter: ino(%v) offset(%v) len(%v)", writer.ino, offset, len(data))
	...
	for dataSize > 0 {
		...
		span.Debugf("TRACE blobStore doBufferWrite: ino(%v) writer.fileSize(%v) writer.fileOffset(%v) writer.blockPosition(%v) position(%v) freeSize(%v)", writer.ino, writer.fileSize, writer.fileOffset, writer.blockPosition, position, freeSize)
		copy(writer.buf[writer.blockPosition:], data[position:position+freeSize])
		span.Debugf("TRACE blobStore doBufferWrite:ino(%v) writer.buf.len(%v)", writer.ino, len(writer.buf))
		position += freeSize
	...
	span.Debugf("TRACE blobStore doBufferWrite Exit: ino(%v) writer.fileSize(%v) writer.fileOffset(%v)", writer.ino, writer.fileSize, writer.fileOffset)
	return size, nil
}
...
func (writer *Writer) writeSlice(ctx context.Context, wSlice *rwSlice, wg bool) (err error) {
	...
	span.Debugf("TRACE blobStore,writeSlice to ebs. ino(%v) fileOffset(%v) len(%v)", writer.ino, wSlice.fileOffset, wSlice.size)
	...
	span.Debugf("TRACE blobStore,location(%v)", location)
	...
	span.Debugf("TRACE blobStore,objExtentKey(%v)", wSlice.objExtentKey)
...
func (writer *Writer) asyncCache(ctx context.Context, ino uint64, offset int, data []byte) {
	span := proto.SpanFromContext(ctx)
	var err error
...
func (writer *Writer) flushWithoutPool(ctx context.Context, inode uint64, flushFlag bool) (err error) {
	...
	span.Debugf("TRACE blobStore flushWithoutPool: ino(%v) buf-len(%v) flushFlag(%v)", inode, len(writer.buf), flushFlag)
	writer.Lock()
	defer func() {
		writer.dirty = false
		writer.Unlock()
...
func (writer *Writer) flush(ctx context.Context, inode uint64, flushFlag bool) (err error) {
	...
	span.Debugf("TRACE blobStore flush: ino(%v) buf-len(%v) flushFlag(%v)", inode, len(writer.buf), flushFlag)
	writer.Lock()
	defer func() {
		writer.dirty = false
		writer.Unlock()
