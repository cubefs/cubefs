...
func NewDataPartitionWrapper(client SimpleClientInfo, volName string, masters []string, preload bool,
	verReadSeq uint64, volStorageClass uint32, volAllowedStorageClass []uint32, innerReq bool,
) (w *Wrapper, err error) {
...
func (w *Wrapper) updateDataPartition(isInit bool) (err error) {
	...
	m2 := make(map[uint32]*proto.StatOfStorageClass)
	for _, st := range dpv.StatByClass {
		m2[st.StorageClass] = st
...
func (w *Wrapper) updateDataNodeStatus() (err error) {
	...
	if err != nil {
		log.LogErrorf("updateDataNodeStatus: get data nodes fail: err(%v)", err)
		return
	}

	newHostsStatus := make(map[string]bool, len(dataNodes))
