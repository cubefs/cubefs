...
func (api *NodeAPI) AddDataNodeWithAuthNode(serverAddr, zoneName, clientIDKey string, disksJsonString string) (id uint64, err error) {
	var request = newAPIRequest(http.MethodGet, proto.AddDataNode)
	request.addParam("addr", serverAddr)
	request.addParam("zoneName", zoneName)
	request.addParam("clientIDKey", clientIDKey)
	request.addParam("disksJsonString", disksJsonString)
	var data []byte
	if data, err = api.mc.serveRequest(request); err != nil {
		return
	}
