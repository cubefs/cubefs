...
func (api *AdminAPI) UnDeleteVolume(volName, authKey string, status bool, clientIDKey string) (err error) {
	var request = newAPIRequest(http.MethodGet, proto.AdminDeleteVol)
	request.addParam("name", volName)
	request.addParam("authKey", authKey)
	request.addParam("clientIDKey", clientIDKey)
...
func (api *AdminAPI) SetClusterParas(batchCount, markDeleteRate, deleteWorkerSleepMs, autoRepairRate,
	loadFactor, maxDpCntLimit string, clientIDKey string) (err error) {
