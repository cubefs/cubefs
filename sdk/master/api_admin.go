...
type IAdminAPI interface {
	...
	DeleteVolume(volName string, authKey string) (err error)
	UpdateVolume(vv *proto.SimpleVolView, txTimeout int64, txMask string, txForceReset bool, txConflictRetryNum int64, txConflictRetryInterval int64, txOpLimit int) (err error)
	...
	CreateVolName(volName string, owner string, capacity uint64, deleteLockTime int64, crossZone bool, normalZonesFirst bool, business string, mpCount int, replicaNum int, size int, volType int, followerRead bool, zoneName string, cacheRuleKey string, ebsBlkSize int, cacheCapacity int, cacheAction int, cacheThreshold int, cacheTTL int, cacheHighWater int, cacheLowWater int, cacheLRUInterval int, dpReadOnlyWhenVolFull bool, txMask string, txTimeout uint32, txConflictRetryNum int64, txConflictRetryInterval int64, optEnableQuota string) (err error)
	CreateDefaultVolume(volName string, owner string) (err error)
	...
	SetForbidMpDecommission(disable bool) (err error)
	SetMetaNodeThreshold(threshold float64) (err error)
	SetClusterParas(batchCount string, markDeleteRate string, deleteWorkerSleepMs string, autoRepairRate string, loadFactor string, maxDpCntLimit string) (err error)
	GetClusterParas() (delParas map[string]string, err error)
	CreatePreLoadDataPartition(volName string, count int, capacity uint64, ttl uint64, zongs string) (view *proto.DataPartitionsView, err error)
