...
clean() {
    docker-compose -f ${RootPath}/docker/docker-compose-multi-zone.yaml down
...
# build
build() {
    docker-compose -f ${RootPath}/docker/docker-compose-multi-zone.yaml run build
}
...
start_servers() {
    isDiskAvailable $DiskPath
    mkdir -p ${DiskPath}/disk/{1..4}
    docker-compose -f ${RootPath}/docker/docker-compose-multi-zone.yaml up -d servers
}

start_client() {
    docker-compose -f ${RootPath}/docker/docker-compose-multi-zone.yaml run client bash -c "/cfs/script/start_client.sh ; /bin/bash"
}

start_monitor() {
    docker-compose -f ${RootPath}/docker/docker-compose-multi-zone.yaml up -d monitor
}
