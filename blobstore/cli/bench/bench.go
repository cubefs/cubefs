...
import (
	...
	bk "github.com/cubefs/cubefs/blobstore/cli/bench/backend"
	ls "github.com/cubefs/cubefs/blobstore/cli/bench/location"
...
// Global variables
var (
	backend, conf, modes, runName, objSize, outputFile, poolName string
...
var (
	...
	locstore        ls.LocationStore
)

var ErrSizeTooLargeForInt64 = fmt.Errorf("size too large to fit in an int64")

func parseSizeToInt64(sizeStr string) (int64, error) {
	sizeUint64, err := humanize.ParseBytes(sizeStr)
	if err != nil {
		return 0, err
	}

	if sizeUint64 > uint64(math.MaxInt64) {
		return 0, ErrSizeTooLargeForInt64
	}

	return int64(sizeUint64), nil
}
...
func init() {
	...
	flagSet.StringVar(&conf, "c", os.Getenv("BENCH_CONF"), "Conf file of blobstore")
	...
	flagSet.StringVar(&objSize, "s", "1M", "Size of objects in bytes with postfix K, M, and G")
	...
	flagSet.Int64Var(&maxErrCnt, "e", DEFAULT_MAX_ERROR_COUNT, "Maximum number of errors allowed <-1 for unlimited>")
...
`
	...
	for _, r := range modes {
		if r != 'p' &&
			r != 'g' &&
			r != 'd' {
			log.Printf("Invalid mode '%s' passed to -m", string(r))
			invalidMode = true
		}
	}
	...
	if size, err = parseSizeToInt64(objSize); err != nil {
		log.Fatalf("Invalid -s argument for object size: %v", err)
	}

	if backend == "blobstore" && dbDir == "" {
...
func prepareData() {
	...
	log.Printf("data size=%d, data MD5=%s", size, objectDataMD5)
}
...
func (is *IntervalStats) makeOutputStats() OutputStats {
	...
	mbps := float64(is.bytes) / seconds / MEGABYTE
...
func (stats *Stats) addOp(threadNum int, bytes int64, latNano int64) {
	if latNano < 0 {
		log.Printf("Negative latency detected: %d ns (thread: %d)", latNano, threadNum)
	}
	// Interval statistics
...
func runPutLoad(ctx context.Context, threadNum int, fendtime time.Time, stats *Stats) {
	...
	for {
		...
		start := time.Now()
		loc, err := store.PutObject(ctx, key, fileobj, fileobj.Size())
		if err != nil {
			errcnt++
			...
			// store key/location info to location store
			if backend == "blobstore" {
				location, _ := loc.ExtractBlobLocation()
				err = locstore.Put(key, location)
				if err != nil {
					errcnt++
					stats.addSlowDown(threadNum)
					atomic.AddInt64(&opCounter, -1)
			...
			if err == nil {
				// Update the stats
				stats.addOp(threadNum, size, latNano)
			}
		}
...
func runGetLoad(ctx context.Context, threadNum int, fendtime time.Time, stats *Stats) {
	...
	for {
		...
		if backend == "blobstore" {
			loc.Value, err = locstore.Get(key)
			if err != nil {
				errcnt++
				stats.addSlowDown(threadNum)
...
func main() {
	...
	case "blobstore":
		...
		if err != nil {
			log.Fatalf("init blobstorage failed: %v", err)
		}
		locstore, err = ls.NewRocksDBMeta(dbDir)
		if err != nil {
			log.Fatalf("init rocksdb metastore failed: %v", err)
		}
	case "dummy":
		store, err = bk.NewDummyStore()
		if err != nil {
			log.Fatalf("init dummy storage failed: %v", err)
		}
	default:
		log.Fatalf("backend %s not supported", backend)
	}

	if err != nil {
		log.Fatalf("init storage error: %v", err)
	}
	...
	for loop := 0; loop < loops; loop++ {
		for _, r := range modes {
			oStats = append(oStats, runWrapper(ctx, loop, r)...)
		}
	}
