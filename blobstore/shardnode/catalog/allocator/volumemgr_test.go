...
func TestGetAllocList(t *testing.T) {
	tp := NewMockAllocTransport(t)
	bid, err := newBidMgr(context.Background(), BlobConfig{BidAllocNums: 10000}, tp)
	require.NoError(t, err)
	...
	//}
	{
		writableVidsArgs := &AllocVolsArgs{
			Fsize:    12 * 1024 * 1024,
			BidCount: 6,
			CodeMode: 2,
	...
	{
		alloc, err := vm.alloc(context.Background(), &AllocVolsArgs{Fsize: 100, CodeMode: 2, BidCount: 1})
		require.NoError(t, err)
...
func BenchmarkVolumeMgr_Alloc(b *testing.B) {
	tp := NewMockAllocTransport(b)
	ctx := context.Background()
	bidMgr, _ := newBidMgr(ctx, BlobConfig{BidAllocNums: 100000}, tp)
	vm := volmgr{tp: tp, bidMgr: bidMgr}
...
func TestAllocVolumeRetry(t *testing.T) {
	...
	startTime := time.Now()
	vols, err := v.allocVolume(ctx, false, codemode.CodeMode(len(codemodes)+1), 1)
