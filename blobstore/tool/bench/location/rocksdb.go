...
func (r *RocksDBStore) Put(key string, loc access.Location) error {
	keyBytes := []byte(key)
...
func (r *RocksDBStore) Get(key string) (access.Location, error) {
	keyBytes := []byte(key)
	...
	if err != nil {
		return access.Location{}, fmt.Errorf("failed to retrieve location from RocksDB: %v", err)
	}
	defer locationBytes.Free()

	if locationBytes.Size() == 0 {
		return access.Location{}, fmt.Errorf("key '%s''s not exist in RocksDB", key)
	}

	// Parse location
	var location access.Location
	if _, err := location.Decode(locationBytes.Data()); err != nil {
		return access.Location{}, fmt.Errorf("failed to parse location: %v", err)
	}
	return location, nil
}
