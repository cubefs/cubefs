...
//
// e.g. to encode the global parity of AzureLrc+1 (k=4, g=2, l=3)
...
// with dimensions of (k+g+l)*k for (k,g,l)-AzureLrc+1
//
// The top (k+g)*k row of the matrix is generated by
// buildMatrixSpecialJerasure to caculate the global parity.
//
// The k+g+1 to k+g+l-1 row is 0-1 vector which is used to
...
//
// The last row is the sum of the k+1 to k+g row, which means
...
func New(dataShards, parityShards int, opts ...Option) (Encoder, error) {
	...
	case r.o.useAzureLrcP1Matrix:
		// we use k,g,l,t to refer the dataShards,globalParityShards,localParityShards and r.Shards
		// we have the following limitations:
		// g*(l-1) = k; k+g+l=t (k,t are known)
		// so we can solve the equations to get:
		// l = ((t+1-k) - sqrt((t+1-k)^2-4*t) )/2
		// g = t-k-l
		t := float64(r.Shards)
		k := float64(dataShards)
		l := int(((t + 1 - k) - math.Sqrt((t+1-k)*(t+1-k)-4*t)) / 2)
