...
func TestChunkData_ConcurrencyWrite(t *testing.T) {
	...
	concurrency := 10
	ioQos, _ := qos.NewIoQueueQos(qos.Config{ReadQueueLen: concurrency, WriteQueueLen: concurrency})
	defer ioQos.Close()
	cd, err := NewChunkData(ctx, core.VuidMeta{}, chunkname, diskConfig, true, ioQos, nil, nil)
	require.NoError(t, err)
	require.NotNil(t, cd)
...
func TestChunkData_Delete(t *testing.T) {
	...
	ioQos, _ := qos.NewIoQueueQos(qos.Config{ReadQueueLen: 100, WriteQueueLen: 100})
	defer ioQos.Close()
	cd, err := NewChunkData(ctx, core.VuidMeta{}, chunkname, diskConfig, true, ioQos, nil, nil)
	require.NoError(t, err)
	require.NotNil(t, cd)
