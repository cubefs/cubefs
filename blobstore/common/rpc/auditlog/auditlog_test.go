...
func initNoContextLengthServer(t *testing.T) (server *httptest.Server, tmpDir string, lc LogCloser) {
	moduleName := "TESTNOCONTEXTLENGTHMOULE"
	tracer := trace.NewTracer(moduleName)
	trace.SetGlobalTracer(tracer)

	tmpDir = os.TempDir() + "/test-NoContextLength-log" + strconv.FormatInt(time.Now().Unix(), 10) + strconv.Itoa(rand.Intn(100000))
	err := os.Mkdir(tmpDir, 0o755)
	require.NoError(t, err)
	...
	noContextLengthHandler := func(w http.ResponseWriter, req *http.Request) {
	...
	entryHandler := func(w http.ResponseWriter, req *http.Request) {
		ah.Handler(w, req, noContextLengthHandler)
	}
...
func TestNoContextLength(t *testing.T) {
	server, tmpDir, lc := initNoContextLengthServer(t)
	defer func() {
		server.Close()
		os.RemoveAll(tmpDir)
		lc.Close()
