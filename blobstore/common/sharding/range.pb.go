...
var fileDescriptor_2a4e1a46defc62fd = []byte{
	...
	0x5c, 0xbc, 0x70, 0x51, 0x8f, 0xc4, 0xe2, 0x0c, 0x01, 0x46, 0x27, 0xc3, 0x1b, 0x0f, 0xe5, 0x18,
	0x3e, 0x3c, 0x94, 0x63, 0x6c, 0x78, 0x24, 0xc7, 0x78, 0xe2, 0x91, 0x1c, 0xe3, 0x85, 0x47, 0x72,
	0x8c, 0x0f, 0x1e, 0xc9, 0x31, 0x4e, 0x78, 0x2c, 0xc7, 0x70, 0xe1, 0xb1, 0x1c, 0xc3, 0x8d, 0xc7,
	0x72, 0x0c, 0x51, 0xdc, 0x7a, 0xfa, 0xd6, 0x30, 0xd7, 0x26, 0xb1, 0x81, 0x03, 0xcc, 0x18, 0x10,
	0x00, 0x00, 0xff, 0xff, 0xa2, 0xf2, 0x85, 0x11, 0x77, 0x01, 0x00, 0x00,
}
...
func (this *Range) GoString() string {
	...
	s := make([]string, 0, 6)
	s = append(s, "&sharding.Range{")
	s = append(s, "Type: "+fmt.Sprintf("%#v", this.Type)+",\n")
	if this.Subs != nil {
		vs := make([]SubRange, len(this.Subs))
		for i := range vs {
			vs[i] = this.Subs[i]
		}
		s = append(s, "Subs: "+fmt.Sprintf("%#v", vs)+",\n")
	}
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *SubRange) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 6)
	s = append(s, "&sharding.SubRange{")
	s = append(s, "Min: "+fmt.Sprintf("%#v", this.Min)+",\n")
	s = append(s, "Max: "+fmt.Sprintf("%#v", this.Max)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func valueToGoStringRange(v interface{}, typ string) string {
...
func (this *Range) String() string {
	...
	repeatedStringForSubs := "[]SubRange{"
	for _, f := range this.Subs {
		repeatedStringForSubs += strings.Replace(strings.Replace(f.String(), "SubRange", "SubRange", 1), `&`, ``, 1) + ","
	}
...
func (this *SubRange) String() string {
	...
	s := strings.Join([]string{`&SubRange{`,
...
func valueToStringRange(v interface{}) string {
	rv := reflect.ValueOf(v)
	if rv.IsNil() {
		return "nil"
	}
