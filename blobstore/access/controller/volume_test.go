...
func TestAccessVolumeGetterNew(t *testing.T) {
	...
	getter, err := controller.NewVolumeGetter(1, proxyService(), proxycli, time.Millisecond*200, 0)
	require.Nil(t, err)
...
func TestAccessVolumeGetterNotExistVolume(t *testing.T) {
	...
	getter, err := controller.NewVolumeGetter(0xfe, proxyService(), proxycli, time.Millisecond*200, 0)
	...
	getter, err = controller.NewVolumeGetter(0xee, proxyService(), proxycli, 0, 0)
	require.NoError(t, err)
	id = vid404
	dataCalled[id] = 0
	for range [10]struct{}{} {
...
func TestAccessVolumePunish(t *testing.T) {
	...
	getter, err := controller.NewVolumeGetter(1, proxyService(), proxycli, 0, 0)
	require.Nil(t, err)
	require.Nil(t, getter.Get(ctx, proto.Vid(0), true))
