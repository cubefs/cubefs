...
type SdkConfig struct {
	access.StreamConfig `json:"stream_config"`
...
func NewSdkBlobstore(conf *SdkConfig) (acapi.API, error) {
	cl := closer.New()
...
func (s *sdkHandler) doGet(ctx context.Context, args *acapi.GetArgs) (resp io.ReadCloser, err error) {
	...
	if !args.IsValid() || !access.VerifyCrcWrap(&args.Location) {
	...
	err = transfer()
	if err != nil {
		access.ReportDownloadWrap(args.Location.ClusterID, "StatusOKError", "-")
...
func (s *sdkHandler) doPutObject(ctx context.Context, args *acapi.PutArgs) (location acapi.Location, hashSumMap acapi.HashSumMap, err error) {
	...
	if err = access.FillCrcWrap(loc); err != nil {
