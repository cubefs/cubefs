...
- 调用[成员移除接口 API ](../../dev-guide/admin-api/blobstore/cm.md)移除宕机的节点；
- 调用[成员添加接口 API ](../../dev-guide/admin-api/blobstore/cm.md)将刚启动的 Clustermgr 节点加到集群中；
- 等待数据自动同步即可
...
调用 clusterMgr 接口设置坏盘，走坏盘修复流程,详细参考[磁盘管理](../../dev-guide/admin-api/blobstore/cm.md)
...
#### 节点故障
1. 机器可用的情况，重启 blobnode 服务，观察能否自动恢复；
