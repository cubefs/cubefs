...
兼容HDFS协议，为Hadoop生态（如Spark、Hive）提供统一数据底座，为计算引擎提供大容量及高吞吐的存储能力。

### 深度训练/机器学习

作为分布式并行文件系统，支撑大规模AI训练、模型存储及分发、IO加速等需求。

### 容器共享存储

容器集群可以将容器镜像的配置文件或初始化加载数据存储在CubeFS上，在容器批量加载时实时读取。多POD间通过CubeFS共享持久化数据，在POD故障时可快速故障切换。
