...
::: tip 提示
目前纠删码子系统的 blobstore-cli 还不够完善，功能覆盖不到70%；后续会不断完善，最终将会实现对于集群各模块接口功能的100%覆盖。
:::
...
是可选配置项，主要配置一些常用变量，
比如 access 接入层服务发现地址，clustermgr 服务地址等。
...
| 命令                      | 描述                            |
|-------------------------|-------------------------------|
| blobstore-cli config    | 管理该 blobstore-cli 内存中的配置项       |
| blobstore-cli util      | 小工具集合，如解析 location、解析时间、生成特定数据 |
