...
var reqRecordMaxCount int
var reqRecordReserveMin int
...
func (records *RequestRecords) GetEvictTimestamp() (evictTimestamp int64) {
	records.rwLock.RLock()
	defer records.rwLock.RUnlock()
	ttlTimestamp := time.Now().Add(-time.Minute * time.Duration(reqRecordReserveMin)).UnixMilli()
	realCnt := records.reqList.Len()
	expectCnt := reqRecordMaxCount
