...
func storeEkSplit(ctx context.Context, mpId uint64, inodeID uint64, ekRef *sync.Map, ek *proto.ExtentKey) (id uint64) {
	span := getSpan(ctx).WithOperation("storeEkSplit")
	if ekRef == nil {
		span.Errorf("mpId [%v] inodeID %v ekRef nil", mpId, inodeID)
		return
	}
	span.Debugf("mpId [%v] inode[%v] dp [%v] extent id[%v] ek [%v]", mpId, inodeID, ek.PartitionId, ek.ExtentId, ek)
	id = ek.PartitionId<<32 | ek.ExtentId
	var v uint32
	if val, ok := ekRef.Load(id); !ok {
		if ek.IsSplit() {
			span.Errorf("mpId [%v]inode id[%v] ek [%v] already be set split", mpId, inodeID, ek)
		}
		v = 1
	} else {
		v = val.(uint32) + 1
	...
	ek.SetSplit(true)
	ekRef.Store(id, v)
	span.Debugf("mpId [%v] inode[%v] dp [%v] extent id[%v].key %v, cnt %v", mpId, inodeID, ek.PartitionId, ek.ExtentId,
		ek.PartitionId<<32|ek.ExtentId, v)
	return
}

func (se *SortedExtents) SplitWithCheck(ctx context.Context, mpId uint64, inodeID uint64, ekSplit proto.ExtentKey, ekRef *sync.Map) (delExtents []proto.ExtentKey, status uint8) {
	span := getSpan(ctx).WithOperation("SplitWithCheck")
	status = proto.OpOk
	endOffset := ekSplit.FileOffset + uint64(ekSplit.Size)
	span.Debugf("mpId [%v]. inode[%v]  ekSplit ek [%v]", mpId, inodeID, ekSplit)
	se.Lock()
	defer se.Unlock()

	if len(se.eks) <= 0 {
		span.Errorf("mpId [%v]. inode[%v] eks empty cann't find ek [%v]", mpId, inodeID, ekSplit)
		status = proto.OpArgMismatchErr
		return
	}
	lastKey := se.eks[len(se.eks)-1]
	if lastKey.FileOffset+uint64(lastKey.Size) <= ekSplit.FileOffset {
		span.Errorf("mpId [%v]. inode[%v] eks do split not found", mpId, inodeID)
		status = proto.OpArgMismatchErr
		return
	}
	...
	if startIndex == 0 {
		status = proto.OpArgMismatchErr
		span.Errorf("mpId [%v]. inode[%v] should have no valid extent request [%v]", mpId, inodeID, ekSplit)
		return
	}
	...
	if !storage.IsTinyExtent(key.ExtentId) && (key.PartitionId != ekSplit.PartitionId || key.ExtentId != ekSplit.ExtentId) {
		status = proto.OpArgMismatchErr
		span.Errorf("mpId [%v] inode[%v]  key found with mismatch extent info [%v] request [%v]", mpId, inodeID, key, ekSplit)
		return
	}
	...
	key.AddModGen()
	if !key.IsSplit() {
		storeEkSplit(ctx, mpId, inodeID, ekRef, key)
	}

	if ekSplit.FileOffset+uint64(ekSplit.Size) > key.FileOffset+uint64(key.Size) {
		status = proto.OpArgMismatchErr
		span.Errorf("mpId [%v] inode[%v] request [%v] out scope of exist key [%v]", mpId, inodeID, ekSplit, key)
		return
	}
	// Makes the request idempotent, just in case client retries.
	if ekSplit.IsEqual(key) {
		span.Warnf("mpId [%v] request key %v is a repeat request", mpId, key)
		return
	}
	...
	storeEkSplit(ctx, mpId, inodeID, ekRef, &delKey)

	if ekSplit.Size == 0 {
		span.Errorf("mpId [%v] inode[%v] delKey %v,key %v, eksplit %v", mpId, inodeID, delKey, key, ekSplit)
	}
	delKey.FileOffset = ekSplit.FileOffset

	delExtents = append(delExtents, delKey)

	span.Debugf("mpId [%v]  inode[%v]  key offset %v, split FileOffset %v, startIndex %v,key [%v], ekSplit[%v] delkey [%v]",
		mpId, inodeID, key.FileOffset, ekSplit.FileOffset, startIndex, key, ekSplit, delKey)
	...
	if key.FileOffset == ekSplit.FileOffset { // at the begin
		...
		if len(se.eks) > 0 {
			...
			span.Debugf("mpId [%v].keyBefore. ek [%v] and ekSplit %v", mpId, keyBefore, ekSplit)
		}
		if keyBefore != nil && keyBefore.IsSequenceWithSameSeq(&ekSplit) {
			span.Debugf("mpId [%v]. inode[%v]  keyBefore [%v], ekSplit [%v]", mpId, inodeID, keyBefore, ekSplit)
			span.Debugf("mpId [%v].merge.head. ek [%v] and %v", mpId, keyBefore, ekSplit)
			keyBefore.Size += ekSplit.Size
		} else {
			se.eks = append(se.eks, ekSplit)
			storeEkSplit(ctx, mpId, inodeID, ekRef, &ekSplit)
		}
		...
		if keyDup.Size == 0 {
			span.Errorf("mpId [%v] inode[%v] delKey %v,keyDup %v, eksplit %v", mpId, inodeID, delKey, keyDup, ekSplit)
		}
		...
		if len(eks) > 0 && ekSplit.IsSequenceWithSameSeq(&eks[0]) {
			span.Debugf("mpId [%v].merge.end. ek [%v] and %v", mpId, ekSplit, eks[0])
		...
		storeEkSplit(ctx, mpId, inodeID, ekRef, mKey)

		if keySize-key.Size-ekSplit.Size == 0 {
			span.Errorf("mpId [%v].inode[%v] keySize %v,key %v, eksplit %v", mpId, inodeID, keySize, key, ekSplit)
		}
		se.eks = append(se.eks, eks...)
	}
	return
...
func (se *SortedExtents) CheckAndAddRef(ctx context.Context, lastKey *proto.ExtentKey, currEk *proto.ExtentKey, addRefFunc func(*proto.ExtentKey)) (ok bool) {
	if !lastKey.IsSameExtent(currEk) {
		return
	}
	span := getSpan(ctx)
	span.Debugf("action[AppendWithCheck.CheckAndAddRef] ek [%v],lastKey %v", currEk, lastKey)
	...
	if lastKey.FileOffset == currEk.FileOffset &&
		...
		span.Debugf("action[AppendWithCheck.CheckAndAddRef] split append key %v", currEk)
		...
		span.Debugf("action[AppendWithCheck.CheckAndAddRef] after split append key %v", currEk)
		if !lastKey.IsSplit() {
			addRefFunc(lastKey)
		}
		addRefFunc(currEk)
...
func (se *SortedExtents) AppendWithCheck(ctx context.Context, inodeID uint64, ek proto.ExtentKey, addRefFunc func(*proto.ExtentKey), clientDiscardExts []proto.ExtentKey) (deleteExtents []proto.ExtentKey, status uint8) {
	status = proto.OpOk
	endOffset := ek.FileOffset + uint64(ek.Size)
	span := getSpan(ctx)
	se.Lock()
	defer se.Unlock()
	span.Debugf("action[AppendWithCheck] ek [%v], clientDiscardExts [%v] se.eks [%v]", ek, clientDiscardExts, se.eks)
	if len(se.eks) <= 0 {
		se.eks = append(se.eks, ek)
		return
	}
	...
	span.Debugf("action[AppendWithCheck] ek [%v],tailKey %v, clientDiscardExts [%v] se.eks [%v]", ek, tailKey, clientDiscardExts, se.eks)
	if ok := se.CheckAndAddRef(ctx, tailKey, &ek, addRefFunc); ok {
		se.eks = append(se.eks, ek)
		return
	}
	...
	if len(invalidExtents) == 1 && invalidExtents[0].Equals(&ek) {
		span.Debugf("action[AppendWithCheck] ek [%v]", ek)
		return
	}
	...
	span.Debugf("action[AppendWithCheck] invalidExtents(%v) deleteExtents(%v) discardExtents(%v)", invalidExtents, deleteExtents, clientDiscardExts)
	if clientDiscardExts != nil {
		if len(deleteExtents) != len(clientDiscardExts) {
			span.Errorf("action[AppendWithCheck] OpConflictExtentsErr error. inode[%v] deleteExtents [%v] clientDiscardExts [%v]", inodeID, deleteExtents, clientDiscardExts)
		...
		for i := 0; i < len(clientDiscardExts); i++ {
			if deleteExtents[i].PartitionId != clientDiscardExts[i].PartitionId || deleteExtents[i].ExtentId != clientDiscardExts[i].ExtentId || deleteExtents[i].ExtentOffset != clientDiscardExts[i].ExtentOffset {
				span.Debugf("action[AppendWithCheck] OpConflictExtentsErr error. inode[%v] idx %v deleteExtents[%v]  clientDiscardExts [%v]", inodeID, i, deleteExtents[i], clientDiscardExts[i])
				return deleteExtents, proto.OpConflictExtentsErr
			}
		}
	} else if len(deleteExtents) != 0 {
		span.Debugf("action[AppendWithCheck] OpConflictExtentsErr error. inode[%v] deleteExtents [%v]", inodeID, deleteExtents)
		return deleteExtents, proto.OpConflictExtentsErr
	}
	...
	defer func() {
		...
		se.CheckAndAddRef(ctx, &se.eks[startIndex-1], &se.eks[startIndex], addRefFunc)
	}()
...
func (se *SortedExtents) Truncate(ctx context.Context, offset uint64, doOnLastKey func(*proto.ExtentKey), insertRefMap func(ek *proto.ExtentKey)) (deleteExtents []proto.ExtentKey) {
	var endIndex int
	...
	if numKeys > 0 {
		...
		if lastKey.FileOffset+uint64(lastKey.Size) > offset {
			...
			*rsKey = *lastKey
			...
			getSpan(ctx).Debugf("SortedExtents.Truncate rsKey %v, deleteExtents %v", rsKey, deleteExtents)
		}
	}
	return
}
