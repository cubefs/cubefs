...
func newTestRocksTree(dir string) (rocksTree *RocksTree) {
	...
	if err = rocksdbHandle.OpenDb(&RocksDBOptions{Dir: dir}); err != nil {
...
func TestInodeTree_Create(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var existIno *Inode
			if existIno, err = inodeTree.Get(test.inode); err != nil {
				t.Errorf(err.Error())
				return
			}

			require.EqualValues(t, test.inode.Inode, existIno.Inode)
...
func TestInodeTreeGet(t *testing.T) {
	...
	var getIno *Inode
	getIno, errForMem = memInodeTree.Get(&Inode{Inode: 30})
	...
	getIno, errForRocks = rocksInodeTree.Get(&Inode{Inode: 30})
	...
	// get not exist inode
	getIno, errForMem = memInodeTree.Get(&Inode{Inode: 1000})
	if errForMem != nil {
	...
	getIno, errForRocks = rocksInodeTree.Get(&Inode{Inode: 1000})
...
func TestInodeTreeRange(t *testing.T) {
	...
	startInode := 43
	index := 0
	memInodeTree.Range(NewInode(43, 0), nil, func(inode *Inode) bool {
	...
	index = 0
	rocksInodeTree.Range(NewInode(43, 0), nil, func(inode *Inode) bool {
		if inode.Inode != uint64(startInode+index) {
			t.Fatalf("Test Range: inode mismatch, expect:%v, actual:%v", startInode+index, inode.Inode)
		}
		index++
...
func TestDentryTreeRange(t *testing.T) {
	...
	memDentryTree.Range(&Dentry{ParentId: 1}, &Dentry{ParentId: 2}, func(den *Dentry) bool {
	...
	actualResult = make([]*Dentry, 0, 6)
	rocksDentryTree.Range(&Dentry{ParentId: 1}, &Dentry{ParentId: 2}, func(den *Dentry) bool {
		actualResult = append(actualResult, den)
		return true
	})
	if len(expectResult) != len(actualResult) {
	...
	memDentryTree.Range(&Dentry{ParentId: 1, Name: "test"}, &Dentry{ParentId: 2}, func(den *Dentry) bool {
		actualResult = append(actualResult, den)
		return true
	})
	if len(expectResult) != len(actualResult) {
	...
	actualResult = make([]*Dentry, 0, 6)
	rocksDentryTree.Range(&Dentry{ParentId: 1, Name: "test"}, &Dentry{ParentId: 2}, func(den *Dentry) bool {
		actualResult = append(actualResult, den)
		return true
	})
	if len(expectResult) != len(actualResult) {
