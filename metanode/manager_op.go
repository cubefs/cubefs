...
func (m *metadataManager) opInsertInnerData(conn net.Conn, p *Packet, remoteAddr string) (err error) {
	var mp MetaPartition
	mp, err = m.getPartition(p.PartitionID)
	if err != nil {
		p.PacketErrorWithBody(proto.OpErr, []byte(err.Error()))
		m.respondToClient(conn, p)
		err = errors.NewErrorf("[%v],err[%v]", p.GetOpMsgWithReqAndResult(), err)
		return
	}
	if !m.serveProxy(conn, mp, p) {
		return
	...
	innDataSt := &proto.InnerDataSt{
		FileOffset: p.KernelOffset,
		Size:       p.Size,
		CRC:        p.CRC,
		Data:       p.Data,
	}
	req := new(InsertInnerDataReq)
	req.Inode = p.ExtentID
	req.InnerData = innDataSt
	err = mp.InsertInnerData(req, p)
	m.respondToClient(conn, p)
	if err != nil {
		log.LogErrorf("%s [opInsertInnerData] insert inner data: %s, "+
			"response to client: %s", remoteAddr, err.Error(), p.GetResultMsg())
	}
	log.LogInfof("%s [opInsertInnerData] req: %d - %v, resp: %v, body: %s",
		remoteAddr, p.GetReqID(), req, p.GetResultMsg(), p.Data)
	return
}

func (m *metadataManager) opGetInnerData(conn net.Conn, p *Packet, remoteAddr string) (err error) {
	req := new(GetInnerDataReq)
	req.FileOffset = p.KernelOffset
	req.Size = uint32(p.ExtentOffset)
	req.Inode = p.ExtentID
	...
	if err != nil {
		p.PacketErrorWithBody(proto.OpErr, []byte(err.Error()))
		m.respondToClient(conn, p)
		err = errors.NewErrorf("[%v],req[%v],err[%v]", p.GetOpMsgWithReqAndResult(), req, err)
		return
	}

	err = mp.GetInnerData(req, p)
	m.respondToClient(conn, p)
	if err != nil {
		log.LogErrorf("%s [opGetInnerData] get inner data: %s, "+
			"response to client: %s", remoteAddr, err.Error(), p.GetResultMsg())
	}
	log.LogInfof("%s [opGetInnerData] req: %d - %v, resp: %v, body: %s",
		remoteAddr, p.GetReqID(), req, p.GetResultMsg(), p.Data)
	return
}
