...
const RocksdbMultipartTestDir = "/tmp/cfs/fsm_multipart_test"
...
func getMpConfigForFsmMultipartTest(storeMode proto.StoreMode) (config *MetaPartitionConfig) {
	...
	if config.StoreMode == proto.StoreModeRocksDb {
		config.RocksDBDir = fmt.Sprintf("%v/%v_%v", RocksdbMultipartTestDir, partitionId, time.Now().UnixMilli())
	}
	return
}
...
func getMultipartForFsmMultipartTest(t *testing.T, mp *metaPartition, key, id string) (multipart *Multipart) {
	multipart, err := mp.multipartTree.Get(&Multipart{key: key, id: id})
...
func TestCleanRocksdbMultipartTestDir(t *testing.T) {
	os.RemoveAll(RocksdbMultipartTestDir)
}
