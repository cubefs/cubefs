...
func TestOpenDb(t *testing.T) {
	...
	err := db.OpenDb(path, 0, 0, 0, 0, 0, 0, 0)
...
func TestReopneDb(t *testing.T) {
	...
	err = db.ReOpenDb(path, 0, 0, 0, 0, 0, 0, 0)
	require.NoError(t, err)
...
func TestOps(t *testing.T) {
	...
	db := metanode.NewRocksdb()
	_ = db.OpenDb(path, 0, 0, 0, 0, 0, 0, 0)
	...
	_ = db2.OpenDb(path2, 0, 0, 0, 0, 0, 0, 0)
	genenerData(t, db2)
	start = time.Now()
	...
	db.ReOpenDb(path, 0, 0, 0, 0, 0, 0, 0)
...
func TestFlushSinglePartition(t *testing.T) {
	path := getRocksdbPathForTest()
	os.RemoveAll(path)
	defer os.RemoveAll(path)
	db := metanode.NewRocksdb()
	...
	err = db.CompactRange([]byte{0}, []byte{255})
	require.NoError(t, err)
