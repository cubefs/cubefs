...
const RocksdbExtendTestDir = "/tmp/cfs/fsm_extend_test"
...
func getMpConfigForFsmExtendTest(storeMode proto.StoreMode) (config *MetaPartitionConfig) {
	...
	if config.StoreMode == proto.StoreModeRocksDb {
		config.RocksDBDir = fmt.Sprintf("%v/%v_%v", RocksdbExtendTestDir, partitionId, time.Now().UnixMilli())
	}
	return
}
...
func checkExtendForFsmExtendTest(t *testing.T, mp *metaPartition, ino uint64, key string, expectedValue []byte) {
	extend, err := mp.extendTree.Get(NewExtend(ino))
	require.NoError(t, err)
...
func TestCleanRocksdbExtendTestDir(t *testing.T) {
	os.RemoveAll(RocksdbExtendTestDir)
}
