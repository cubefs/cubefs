...
type nvmeIdCtrl struct {
	vId        uint16 //Only the first two bytes of vid are used, so there is no complete description of the struct. The nvme_
...
func GetDevCheckSfx(path string) (isSfx bool, devName string) {
	...
	ioctlCmd.opcode = NVME_ADMIN_IDENTIFY //nvme_admin_identify
	ioctlCmd.nsid = 0
