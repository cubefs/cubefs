...
type ExtentApi interface {
	...
	Write(inode uint64, offset int, data []byte, flags int, checkFunc func() error, storageClass uint32, isMigration, waitForFlush bool, retainForAsyncRelease func() func()) (write int, err error)
...
func (t *TransitionMgr) migrate(e *proto.ScanDentry) (err error) {
	...
	for {
		...
		if readN > 0 {
			writeN, err = t.ecForW.Write(e.Inode, writeOffset, buf[:readN], 0, nil, proto.OpTypeToStorageType(e.Op), true, false, nil)
			if err != nil {
