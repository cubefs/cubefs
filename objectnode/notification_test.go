...
func TestParseNotificationConfig(t *testing.T) {
	...
	mgr := NewNotificationMgr()
	mgr.AddNotifier(notifier1, notifier2)
	...
	for _, tc := range testCases {
		_, err = ParseNotificationConfig([]byte(tc.Value), "cfs-test", mgr)
		if err != nil {
...
func TestEventFilterMatch(t *testing.T) {
	...
	notification, err := ParseNotificationConfig([]byte(config), "cfs-test", mgr)
	require.NoError(t, err)
