...
func (m *NotificationMgr) Close() error {
	...
	for _, notifier := range m.notifiers {
		if notifier != nil {
			notifier.Close()
			delete(m.notifiers, notifier.ID())
		}
	}
...
func SendEventNotification(notifyMgr *NotificationMgr, vol *Volume, eventParams *EventParams) {
	if notifyMgr == nil || vol == nil || eventParams == nil {
		return
	}

	notifierIDSet := vol.EventMatch(eventParams.Name, eventParams.Key)
	if len(notifierIDSet) <= 0 {
		return
	}

	reqParameters := map[string]string{
		"host":            eventParams.Request.Host,
		"userAgent":       eventParams.Request.UserAgent(),
		"sourceIPAddress": getRequestIP(eventParams.Request),
	}

	reqParam := ParseRequestParam(eventParams.Request)
	respElements := map[string]string{
		"x-amz-request-id": reqParam.RequestID(),
	}
	if eventParams.Response.Header().Get(ContentLength) != "" {
		respElements["content-length"] = eventParams.Response.Header().Get(ContentLength)
	}
	if eventParams.Response.Header().Get(ContentType) != "" {
		respElements["content-type"] = eventParams.Response.Header().Get(ContentType)
	}
	...
	event := &Event{
		...
		AwsRegion:         eventParams.Region,
		EventTime:         eventTime.Format(ISO8601Layout),
		EventName:         strings.TrimPrefix(string(eventParams.Name), "s3:"),
		...
		S3: S3Record{
			...
			Bucket: BucketRecord{
				Name:          eventParams.Bucket,
				OwnerIdentity: IdentityRecord{PrincipalID: reqParam.AccessKey()},
				ARN:           "arn:aws:s3:::" + eventParams.Bucket,
			},
			Object: ObjectRecord{
				Key:       url.QueryEscape(eventParams.Key),
				Sequencer: fmt.Sprintf("%X", eventTime.UnixNano()),
			},
		},
	}

	if eventParams.FileInfo != nil {
		event.S3.Object.Size = eventParams.FileInfo.Size
		event.S3.Object.ETag = eventParams.FileInfo.ETag
		event.S3.Object.ContentType = eventParams.FileInfo.MIMEType
		if len(eventParams.FileInfo.Metadata) > 0 {
			event.S3.Object.UserMetadata = make(map[string]string)
			for k, v := range eventParams.FileInfo.Metadata {
